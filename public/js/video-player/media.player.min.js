...
function loadBuffer(){
	if(mediaPlayer.buffered.length > 0){
		...
		$('#buffered').css({'width': percentLoaded-1 + '%'});
...
function hasError(){
 	$(loader).html('An error occured please try agian later.').css({'color':'#cc1c1c'})
}
...
function changeVidQuality(mp4Ext,webmExt,oggExt){
    ...
    //mediaPlayer.addEventListener('canplay', hideLoader, false);
    //mediaPlayer.addEventListener('timeupdate', seekTimeUpdate, false);
}
function highQuality(e){
	e.preventDefault();$('#play-icon').fadeOut('fast');$(loader).fadeIn('fast');$(highQual).css({'color':'#1ee004'});$(normalQual).css({'color':'#fff'});$(lowQual).css({'color':'#fff'});
...
function normalQuality(e){
	e.preventDefault();$('#play-icon').fadeOut('fast');$(loader).fadeIn('fast');$(highQual).css({'color':'#fff'});$(normalQual).css({'color':'#1ee004'});$(lowQual).css({'color':'#fff'});
	changeVidQuality('.mp4','.webm','.ogg');
}
function lowQuality(e){
	e.preventDefault();$('#play-icon').fadeOut('fast');$(loader).fadeIn('fast');$(highQual).css({'color':'#fff'});$(normalQual).css({'color':'#fff'});$(lowQual).css({'color':'#1ee004'});
...
function seekTimeUpdate(){
	...
	if(floor(mediaPlayer.currentTime) == adsTime){
...
function checkVid(){
	if(isNaN(durSec)){
		$('.ctime').html('00:00');
		$('#error-video').html('Video is not ready please wait...').css({'color':'#fff'});
		$('#error-video').fadeIn();	
		$(loader).fadeIn('fast');	
	}else{
		$('#error-video').fadeOut();	
...
$('html, body').mousemove(function(){
	if(mouseOverPlayer){$('#controls').fadeIn(500);}else{$('#controls').fadeOut('fast');}
});
