			...
			return {
					show: function(){
							  for(var i = 0; i < countAnnotation; i++){
							  	...
							  	if(Math.floor(mediaPlayer.currentTime)==startTime) {$('#close-'+i).fadeIn();}//document.getElementById('annotation').style.display = 'block';
							  	if(Math.floor(mediaPlayer.currentTime)==endTime) {$('#close-'+i).fadeOut();}//document.getElementById('annotation').style.display = 'none';
							 }
						}
					}
		}();
...
$('#media-video, #controls, .play-icon, #ads-hide').mouseover(function(){
	mouseOverPlayer = true;
	//$('.ads-absolute-wrapper').css({'bottom':'50px'});
});
$('#media-video, #controls, .play-icon').mouseleave(function(){
	mouseOverPlayer = false;
	//$('.ads-absolute-wrapper').css({'bottom':'0px'});
});
$('html, body').mousemove(function(){
	if(mouseOverPlayer){$('#controls').fadeIn(500);hideControls=false;$('#ads-hide').css({'bottom':'50px'});}else{$('#controls').fadeOut('fast');hideControls=true;$('#ads-hide').css({'bottom':'0px'});}
