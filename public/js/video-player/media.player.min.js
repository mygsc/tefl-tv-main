...
var mediaPlayer, seen = false, countAnnotation=0,count=0,  duration = $('#duration').val(), autoplay = $('#autoplay').val(), filename = $('#filename').val(), percentageloaded, handle, timeout, progressHandle=false, hideControls = false, mouseOverPlayer =false, progWidth, mp4, webm, vidMp4Src, vidWebmSrc, buffPercent, percentLoaded, loader, progress, hrs=0, mins=0, secs=0, tmpSecs=0, adsTime = 10, ads=0, vidMinLenght=0, vidSecLenght=0, videoCurrentTime=0,playPauseBtn, timeDuration=0,muteBtn, playIcon = false, replay, _time, curHrs, curMin, curSec, durHrs, durMin, durSec,progressBar, soundHover = false, volumeHover = false, currentTime=0, videoPlaying = false, start = false, error = false,videoTimeLenght,volumes=0, volumeClick = false, mouseX = 0, mouseY = 0, volumeY=0, volumeDrag = false, progressbarClick = false,updProgWidth = 0, videoControls, volumeStatus, bufferedAmount, currentBuffered, currentProgress, playBtn, play, seekSlider,highQual,normalQual, lowQual, fullscreenVid, mouseMoving=false;
...
function toggleMute() {
	...
	if (mediaPlayer.muted) {
		...
		$('#volume-vertical').css({'height':  '100%'});
	}
	else {
		changeButtonType(muteBtn,'player sound-off', 'Unmute');
		mediaPlayer.muted = true;
		$('#volume-vertical').css({'height':'0%'});
	}
}
...
function adjustVolume(e){
	...
	$('#volume-vertical').css({'height' : mouseY+'%'});
	$('#volume-button').css({'top' : $('#volume-vertical').height()+'%'});
	mediaPlayer.volume = mouseY/100;//$('#volume-vertical').height() / $(this).height();
...
$('#media-video').bind('waiting', function(){
	buffering();
});
...
$('#buffered').bind('click', function(e) {	
	videoForwardBackward(e);
});
$('#current-progress').bind('click', function(e) {	
	videoForwardBackward(e);
});
...
$('span.close-ads').bind('click', function(){
