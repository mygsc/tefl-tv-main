...
function highQuality(e){
	...
	// var getSource = source.src;
	// var lowSrc = getSource.replace('_hd.mp4', '_low.mp4');
	// var normalSrc = lowSrc.replace('_low.mp4', '.mp4');
	// var highSrc = normalSrc.replace('.mp4', '_hd.mp4');
...
function lowQuality(e){
	...
	var source = document.getElementById('vid-src');
	var getSource = source.src;
	var highSrc = getSource.replace('_low.mp4', '_hd.mp4');
	var normalSrc = highSrc.replace('_hd.mp4', '.mp4');
	var lowSrc = normalSrc.replace('.mp4', '_low.mp4');
	if(videoPlaying == true){
		currentTime = seekSlider.value;
		source.setAttribute('src',lowSrc);
		mediaPlayer.load();
    	mediaPlayer.play();
	}
}
