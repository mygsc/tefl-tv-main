...
function seekTimeUpdate(){
	updProgWidth = Math.floor((mediaPlayer.currentTime / mediaPlayer.duration) * progWidth);
	//updProgWidth = (updProgWidth / progWidth) * 100;
	$('#current-progress').css({'width' : updProgWidth+'px'});
	$('#button-progress').css({'left' : (updProgWidth-22)+'px'});
...
$('#current-progress').bind('mousedown', function(e) {	
	videoForwardBackward(e);
});
...
$('html, body').mousemove(function(e){
	if(progressHandle==true){
		...
		if(currentTime>mediaPlayer.currentTime) {mediaPlayer.currentTime += mouseX;}
		else {mediaPlayer.currentTime -= mouseX;}
	}
});
