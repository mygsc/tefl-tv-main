...
<<<<<<< HEAD
var mediaPlayer, progWidth, progress, hrs=0, mins=0, secs=0, tmpSecs=0, adsTime = 7, ads=0, vidMinLenght=0, vidSecLenght=0, videoCurrentTime=0,playPauseBtn, timeDuration=0,muteBtn, playIcon = false, replay, _time, curHrs, curMin, curSec, durHrs, durMin, durSec,progressBar, soundHover = false, volumeHover = false, currentTime=0, videoPlaying = false, start = false, error = false,videoTimeLenght,volumes=0, volumeClick = false, mouseX = 0, mouseY = 0, volumeY=0, volumeDrag = false, progressbarClick = false,updProgWidth = 0, videoControls, volumeStatus, bufferedAmount, currentBuffered, currentProgress, playBtn, play, seekSlider,highQual, lowQual, fullscreenVid, mouseMoving=false,videoQuality = {'9001p':'highres', '1080p':'hd1080', '720p':'hd720', '480p':'large', '360p':'medium', '240p':'small', '144p':'tiny'};
=======
var mediaPlayer, progWidth, retry, mp4, webm, ogg, vidScr, buffPercent, percentLoaded, loader, progress, hrs=0, mins=0, secs=0, tmpSecs=0, adsTime = 7, ads=0, vidMinLenght=0, vidSecLenght=0, videoCurrentTime=0,playPauseBtn, timeDuration=0,muteBtn, playIcon = false, replay, _time, curHrs, curMin, curSec, durHrs, durMin, durSec,progressBar, soundHover = false, volumeHover = false, currentTime=0, videoPlaying = false, start = false, error = false,videoTimeLenght,volumes=0, volumeClick = false, mouseX = 0, mouseY = 0, volumeY=0, volumeDrag = false, progressbarClick = false,updProgWidth = 0, videoControls, volumeStatus, bufferedAmount, currentBuffered, currentProgress, playBtn, play, seekSlider,highQual,normalQual, lowQual, fullscreenVid, mouseMoving=false,videoQuality = {'9001p':'highres', '1080p':'hd1080', '720p':'hd720', '480p':'large', '360p':'medium', '240p':'small', '144p':'tiny'};
>>>>>>> f13f8d28743a1ee7a73ce02eaefedbcd59be2048
function GSCMediaPlayer(){
	mp4 =  document.getElementById('mp4'); 
	webm =  document.getElementById('webm'); 
	ogg =  document.getElementById('ogg'); 
	vidSrc = mp4.src; 
	vidSrc = vidSrc.replace('.mp4',''); 
...
function loadBuffer(){
	if(mediaPlayer.buffered.length > 0){
			...
			percentLoaded = Math.floor((buffPercent / timeDuration) * 100);
...
=======
			...
			// 	$(loader).fadeIn();
			// }
			if(seekSlider.value >= percentLoaded){$(loader).fadeIn();}else{$(loader).fadeOut();}
		}
	else{console.log('no buffer recieved...');$(loader).fadeOut('fast');}
>>>>>>> f13f8d28743a1ee7a73ce02eaefedbcd59be2048
}
...
$('#media-video').bind("contextmenu", function(){return false;});

//After play then pause the video more consistent in cross browser...
$('#media-video').bind('ended', function(){if(!this.paused) this.pause();$('.advertisement').fadeOut(1000);});
...
<<<<<<< HEAD
	...
	if(videoPlaying == true){
		...
		source.setAttribute('src','/videos/2-gil/v5vOskR0vxy/v5vOskR0vxy_hd.mp4');
		mediaPlayer.load();
...
function lowQuality(e){
	...
	if(videoPlaying == true){
		currentTime = seekSlider.value;
		source.setAttribute('src','/videos/2-gil/v5vOskR0vxy/v5vOskR0vxy_low.mp4');
		mediaPlayer.load();
		mediaPlayer.currentTime = currentTime;
    	mediaPlayer.play();
	}
=======
	e.preventDefault();$('#play-icon').fadeOut('fast');$(loader).fadeIn('fast');$(highQual).css({'color':'#1ee004'});$(normalQual).css({'color':'#fff'});$(lowQual).css({'color':'#fff'});var hd;
	...
	if(mediaPlayer.canPlayType("video/mp4")){ hd = vidSrc+'_hd.mp4';mp4.setAttribute('src', hd); mp4.addEventListener('error', checkVideo, false);}
    else if(mediaPlayer.canPlayType("video/webm")){ hd = vidSrc+'_hd.webm';webm.setAttribute('src', hd);webm.addEventListener('error', checkVideo, false);}
    else if(mediaPlayer.canPlayType("video/ogg")){hd = vidSrc+'_hd.ogg';ogg.setAttribute('src', hd);ogg.addEventListener('error', checkVideo, false);}
    else{window.alert("Can't play video source.");}mediaPlayer.pause();mediaPlayer.load();
    mediaPlayer.addEventListener('canplaythrough', togglePlayPause, false);
    mediaPlayer.addEventListener('timeupdate', seekTimeUpdate, false);     	
}
function normalQuality(e){
	e.preventDefault();$('#play-icon').fadeOut('fast');$(loader).fadeIn('fast');$(highQual).css({'color':'#fff'});$(normalQual).css({'color':'#1ee004'});$(lowQual).css({'color':'#fff'});var normal;
	if(mediaPlayer.canPlayType("video/mp4")){normal = vidSrc+'.mp4';mp4.setAttribute('src', normal);mp4.addEventListener('error', checkVideo, false);}
    else if(mediaPlayer.canPlayType("video/webm")){normal = vidSrc+'.webm';webm.setAttribute('src', normal);webm.addEventListener('error', checkVideo, false);}
    else if(mediaPlayer.canPlayType("video/ogg")){normal = vidSrc+'.ogg';ogg.setAttribute('src', normal);ogg.addEventListener('error', checkVideo, false); }
    else{window.alert("Can't play video source.");}mediaPlayer.pause();mediaPlayer.load();
    mediaPlayer.addEventListener('canplaythrough', togglePlayPause, false);	
    mediaPlayer.addEventListener('timeupdate', seekTimeUpdate, false);      
}
function lowQuality(e){
	e.preventDefault();$('#play-icon').fadeOut('fast');$(loader).fadeIn('fast');$(highQual).css({'color':'#fff'});$(normalQual).css({'color':'#fff'});$(lowQual).css({'color':'#1ee004'});var low;
	if(mediaPlayer.canPlayType("video/mp4")){low = vidSrc+'_low.mp4';mp4.setAttribute('src', low);mp4.addEventListener('error', checkVideo, false);}
    else if(mediaPlayer.canPlayType("video/webm")){low = vidSrc+'_low.webm';webm.setAttribute('src', low);webm.addEventListener('error', checkVideo, false);}
    else if(mediaPlayer.canPlayType("video/ogg")){low = vidSrc+'_low.ogg';ogg.setAttribute('src', low);ogg.addEventListener('error', checkVideo, false); }
    else{window.alert("Can't play video source.");}mediaPlayer.pause();mediaPlayer.load();
    mediaPlayer.addEventListener('canplaythrough', togglePlayPause, false);	
    mediaPlayer.addEventListener('timeupdate', seekTimeUpdate, false); 
}
