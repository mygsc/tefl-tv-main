...
protected $video_;
	...
	public function getEditVideo($file_name, $tags = null, $category = null){
		...
		if(!$video->isEmpty() || Auth::User()->id != $video->first()->user_id){
			...
			return View::make('users.updatevideos', compact('countSubscribers','usersChannel','usersVideos', 'findUsersVideos','countAllViews', 'countVideos','video','tags','owner','picture','hms', 'thumbnail','videoCategory'));
		}
	...
	public function postFeedbacks() {
		...
		foreach($query as $q) {
			$channelNames[] = array('id' => $q->id,'label' => $q->channel_name);
		}
